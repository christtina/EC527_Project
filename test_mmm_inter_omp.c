...
int main(int argc, char *argv[])
{
  ...
  void mmm_ijk_blocked(matrix_ptr a, matrix_ptr b, matrix_ptr c, long int block_size);
  ...
  for (i = 0; i < ITERS; i++) {
    ...
    set_matrix_length(c0,BASE+(i+1)*DELTA);
    clock_gettime(CLOCK_REALTIME, &time1);
    mmm_ijk_blocked(a0,b0,c0,4);
    clock_gettime(CLOCK_REALTIME, &time2);
    time_stamp[OPTION][i] = diff(time1,time2);
    printf("\niter = %lu", i);
  }

  printf("\nlength, ijk, kij, jki, mmm_blocked");
  for (i = 0; i < ITERS; i++) {
    printf("\n%lu, ", BASE+(i+1)*DELTA);
    for (j = 0; j < OPTIONS; j++) {
      if (j != 0) printf(", ");
...
void mmm_ijk_blocked(matrix_ptr a, matrix_ptr b, matrix_ptr c, long int block_size)
{
  ...
  data_t zero;
  omp_set_num_threads(4);
#pragma omp parallel shared(a0,b0,c0,length) private(i,j,k,kk,jj,sum) 
{
#pragma omp for
  for (i = 0; i < length; i++)
  {
